...
void Preprocess::robosense_handler(const sensor_msgs::PointCloud2::ConstPtr &msg)
{
  ...
  // DEBUG: 打印字段信息一次
  // ========================
  static bool show_fields = true;
  if (show_fields)
  {
    std::cout << "\n[INFO] PointCloud2 Fields in Message:" << std::endl;
    for (const auto& field : msg->fields)
    {
      std::cout << "  - name: " << field.name << ", datatype: " << static_cast<int>(field.datatype)
  ...
  for (int i = 0; i < plsize; ++i)
  {
    ...
    // timestamp → curvature（ms）
    added_pt.curvature = (pt.timestamp - msg->header.stamp.toSec()) * 1000.0;

    double dist = added_pt.x * added_pt.x + added_pt.y * added_pt.y + added_pt.z * added_pt.z;
    if (dist < blind_sqr || std::isnan(added_pt.x) || std::isnan(added_pt.y) || std::isnan(added_pt.z))
      continue;

    if (feature_enabled)
    {
      int layer = static_cast<int>(pt.ring);
      if (layer < 0 || layer >= N_SCANS)
      {
        std::cout << "[WARN] Invalid ring index: " << layer << std::endl;
        continue;
      }
  ...
  if (feature_enabled)
  {
    for (int j = 0; j < N_SCANS; ++j)
    {
      ...
      for (int i = 0; i < linesize - 1; ++i)
      {
        ...
        float dx = pl[i].x - pl[i + 1].x;
        float dy = pl[i].y - pl[i + 1].y;
        float dz = pl[i].z - pl[i + 1].z;
        types[i].dista = dx * dx + dy * dy + dz * dz;
      }
      types[linesize - 1].range = std::sqrt(pl[linesize - 1].x * pl[linesize - 1].x +
